...
local siriusValues = {
    ...
    rotateWhileEnabled = false,
    callback = function(value)
        -- Thay đổi tốc độ chạy khi bật/tắt
        local player = game.Players.LocalPlayer
        local character = player.Character
        if character and character:FindFirstChild("Humanoid") then
            local humanoid = character:FindFirstChild("Humanoid")
            if value then
                -- Tăng tốc độ chạy gấp 3 lần
                humanoid.WalkSpeed = 16 * 3  -- Mặc định là 16, tăng lên gấp 3
            else
                -- Đặt lại tốc độ chạy về mặc định
                humanoid.WalkSpeed = 16
            end
        end
        
        -- Cập nhật hiển thị ESP
        for _, highlight in ipairs(espContainer:GetChildren()) do
            highlight.Enabled = value
        end
    end,
